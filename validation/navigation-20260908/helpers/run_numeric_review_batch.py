    ...
    try:
        ...
        if freeze.exists():
            ...
            identity = {'schema': 1, 'round_identity': manifest['round_identity'],
                        ...
                        'resources': {'workers': quota['workers'], 'cores_per_worker': quota['threads_per_worker'],
                                      'gib_per_worker': quota['memory_gib_per_worker'],
                                      'total_cores': quota['aggregate_cpu_cores'], 'total_gib': quota['aggregate_memory_gib'],
                                      'core_slot_offset': quota.get('core_slot_offset', 0)},
        ...
        quota = identity['resources']
        offset = quota.get('core_slot_offset', 0)
        if (not 1 <= workers <= 4 or threads < 1 or offset < 0
                or not workers * threads <= quota['total_cores'] <= 12 - offset
                or not 0 < workers * quota['gib_per_worker'] <= quota['total_gib'] <= 128):
            raise ValueError('Review worker resources exceed their frozen aggregate allocation')
...
def main():
    ...
    result = run_tree([PYTHON, '-u', frozen, '--round', root, '--review-id', args.review_id, '--controller'],
                      directory, env, sum(physical_core_masks(offset + cores)[offset:]), quota['aggregate_memory_gib'], 7 * 86400)
