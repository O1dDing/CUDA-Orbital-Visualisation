...
failures=[]; reports=[]; bundles=Counter(); probes=0; linear_clicks=Counter()
for name,digest in view['case_reports'].items():
    ...
    for click in result.get('linear_member_clicks',[]):
        linear_clicks['pass' if click['pass'] else 'fail']+=1
result={'round_identity':identity,'production_commit':manifest['git_commit'],
    ...
    'linear_member_click_counts':dict(linear_clicks),
    ...
    'timing':{**numeric['timing'],'view_review_wall_seconds':view['wall_seconds'],
        'OLD-015':next((c for c in numeric['cases'] if c['case_id']=='OLD-015'),None)},
