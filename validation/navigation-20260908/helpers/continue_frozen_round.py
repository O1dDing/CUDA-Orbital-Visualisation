...
def status(stage):
    atomic(root/'pipeline-progress.json',{'round_identity':identity,'stage':stage,
        ...
        'reference_workflow':'independently managed; physical acceptance pending; no Gaussian jobs controlled here'})
def run(name,argv):
...
result={'round_identity':identity,'wall_seconds':time.time()-start,'steps':steps,
    ...
    'reference_workflow':'independently managed; physical acceptance pending; no Gaussian jobs controlled here','formal_case_passes':0,
