...
root = workspace / 'outputs/cov-complete-validation-20260906/external-coordinate-preparation-v1'
...
for candidate in json.loads(source.read_text(encoding='utf-8'))['records']:
    ...
    if candidate['lookup_status'] != 'unique-metadata-match':
        ...
        matches = [item for item in candidate['identity_candidates'] if item.get('formula_charge_consistent')]
